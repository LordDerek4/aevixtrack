...
import { daysUntil, formatCurrency, serviceInitials } from "@/lib/utils";
  ...
  return (
    <div className="mt-4">
      ...
      <div className="h-2 rounded-full bg-white/[0.08]">
        <div className="h-full rounded-full bg-fog" style={{ width: `${Math.max(4, progress)}%` }} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="mt-8 flex min-h-[480px] flex-col items-center justify-center rounded-[32px] border border-white/10 bg-white/[0.02] p-12 text-center">
      ...
      <p className="mt-2 max-w-sm text-sm leading-6 text-white/50">
        Could not reach the database. Check your Supabase connection in{" "}
        <code className="rounded bg-white/10 px-1 py-0.5 text-xs">.env.local</code> and try again.
      </p>
...
export function DashboardClient() {
  ...
  const stats = useMemo(() => {
    ...
    return { monthly, yearly, upcoming, trials, active: active.length, cancelled };
  }, [subscriptions]);
  ...
  async function saveSubscription(values: SubscriptionInput) {
    ...
    if (target) {
      // Edit — optimistic update, revert on failure
      ...
      // Create — optimistic add with temp ID, replace on success or remove on failure
    ...
    <>
      ...
      {loading ? (
        ...
        <>
          ...
          <section className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {[
              { label: "Monthly spend", value: fc(stats.monthly), icon: CircleDollarSign },
              { label: "Yearly spend", value: fc(stats.yearly), icon: CalendarClock },
              { label: "Active subscriptions", value: String(stats.active), icon: CreditCard },
              { label: "Trials ending soon", value: String(stats.trials.length), icon: Sparkles }
            ].map((metric) => {
              ...
              return (
                <Card key={metric.label}>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="grid size-10 place-items-center rounded-2xl bg-white/[0.08]">
                      <Icon className="size-5 text-fog" />
                    </span>
                  </div>
                  <p className="mt-8 text-3xl font-semibold tracking-[-0.04em]">{metric.value}</p>
                </Card>
              );
            })}
          </section>
                ...
                {categories.map((cat) => {
                  ...
                  return (
                    <div key={cat} className="grid gap-2">
                      ...
                      <div className="h-2 rounded-full bg-white/[0.08]">
                        <div className="h-full rounded-full bg-gradient-to-r from-white to-fog" style={{ width: `${width}%` }} />
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="grid gap-3">
                {stats.upcoming.length ? (
                  stats.upcoming.map((s) => (
                    <div key={s.id} className="flex items-center justify-between rounded-2xl bg-white/[0.04] p-3">
                      <div className="flex items-center gap-3">
                        <LogoBadge name={s.serviceName} />
                        <div>
                          <p className="text-sm font-medium">{s.serviceName}</p>
                          <p className="text-xs text-white/[0.42]">{daysUntil(s.renewalDate)} days</p>
                        </div>
                      </div>
                      <p className="text-sm text-white/70">{fc(Number(s.cost))}</p>
                    </div>
                  ))
                ) : (
                  <p className="rounded-2xl bg-white/[0.04] p-5 text-sm text-white/[0.46]">No renewals in the next 14 days.</p>
                )}
              </div>
            </Card>
          </section>
              ...
              <CardHeader className="flex-col md:flex-row">
                <div>
                  ...
                  <p className="mt-1 text-sm text-white/[0.46]">
                    Search, filter, edit, mark cancelled, archive, or delete tracked services.
                    {stats.cancelled ? ` ${stats.cancelled} cancelled item${stats.cancelled === 1 ? "" : "s"} excluded from spend.` : ""}
                  </p>
                </div>
              ...
              {filteredSubscriptions.length ? (
                <div className="grid gap-3">
                  {filteredSubscriptions.map((s) => (
                    <article key={s.id} className="rounded-[24px] border border-white/10 bg-white/[0.035] p-4 transition hover:bg-white/[0.06]">
                      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div className="flex min-w-0 items-center gap-4">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-sm text-white/[0.46]">
                              Renews {new Date(s.renewalDate).toLocaleDateString()} · {s.billingCycle.toLowerCase()}
                            </p>
